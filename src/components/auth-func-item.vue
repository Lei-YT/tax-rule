        ...
        <div class="menu-item" :key="vo.id">
          <p class="title" flex="cross:center">
            <span flex-box="1" flex="cross:center">
              ...
              />
              {{vo.name}}
              <span style="color:#999;font-size:12px;margin-left:10px" v-if="vo.components">（{{vo.components}}）</span>
            </span>
            <span flex-box="0">
...
<script>
import addModel from "@/views/auth-func-add.vue";
import editModel from "@/views/auth-func-edit.vue";
export default {
  name: "MenuItem",
  ...
  methods: {
    del(index, id) {
      ...
      _this.$Modal.confirm({
        ...
        onOk: () => {
          _this.$http.delete(`/func/delete/${id}`).then(res => {
            if (res) {
