<template>
  <div v-cloak>
    <Modal v-model="isShow" width="460" :mask-closable="false" @on-cancel="cancel">
      <p slot="header">
        <b v-if="publishType==1">发布</b>
        <b v-if="publishType==0">停用</b>
      </p>
...
export default {
  ...
  methods: {
    getAuditUser() {
      var _this = this;
      _this.$http.post(`/item/getAuditUser?projectId=${_this.$route.query.projectId}`).then(res => {
        if (res) {
    ...
    save() {
      ...
      _this.$http.post(`/edition/updateStatus`, _this.formData).then(res => {
        if (res) {
          _this.isShow = false;
