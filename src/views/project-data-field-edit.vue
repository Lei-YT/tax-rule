...
<script>
export default {
  props: ["open", "projectId", "data", "tableType"],
  data() {
    return {
      isShow: false,
      formData: {
  ...
  methods: {
    save() {
      ...
      _this.$http
        .post(`/data/updateField?projectId=${_this.projectId}`, _this.formData)
        .then((res) => {
          if (res) {
            _this.isShow = false;
            _this.$parent.openFieldEditModel = false;
    ...
    cancel() {
      this.$parent.openFieldEditModel = false;
    },
  },
};
</script>
